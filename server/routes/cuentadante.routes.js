...
router.post('/requests/create', async (req, res) => {
    const { applicant_name, applicant_position, asset_id, reason, priority, expected_return_date } = req.body;
    
    try {
        // Validar campos requeridos
        if (!applicant_name || !asset_id || !reason || !expected_return_date) {
            return res.status(400).json({ 
                error: 'Faltan campos requeridos: applicant_name, asset_id, reason, expected_return_date' 
            });
        }
        ...
        if (asset.status !== 'Available') {
            return res.status(400).json({ error: 'El bien no está disponible' });
        }
        ...
        const result = await query(`
            ...
            RETURNING *
        `, [applicant_name, applicant_position, asset_id, reason, priority || 'Media', expected_return_date]);
